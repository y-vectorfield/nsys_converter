...
class NsysConverter:
    ...
    def convert(self, column_name: str) -> None:
        try:
            file = pd.read_excel(
                self.file_path, sheet_name=self.sheet_name, header=0)
        ...
        if self.save_file_name is not None:
            # 指定のファイル名でファイルパスを構築
            save_path = self.file_path.parent / \
...
if __name__ == '__main__':
    ...
    parser.add_argument('--file_path', type=str, required=True,
                        help='File path of Nsyight Systems')
    parser.add_argument('--sheet_name', type=str,
                        default='Sheet1', help='Sheet name of result file')
    parser.add_argument('--column_name', type=str,
                        required=True, help='Target column name for convert')
